...
class Server:
    ...
    def server_reroute(self, host, conn, port):
        ...
        if data[4:] == 'server':
            self.server_to_server(host, conn, int(data[:4]))
        else:
    ...
    def connect_to_other_servers(self):
        message = str(self.port) + "server"
        if self.port == self.ports[1]:   
            self.sockets[0].connect((self.host, self.ports[0]))
            self.sockets[0].sendto(message.encode(), (self.host, self.ports[0]))
        elif self.port == self.ports[2]:
            self.sockets[0].connect((self.host, self.ports[0]))
            self.sockets[0].sendto(message.encode(), (self.host, self.ports[0]))
            self.sockets[1].connect((self.host, self.ports[1]))
            self.sockets[1].sendto(message.encode(), (self.host, self.ports[1]))
